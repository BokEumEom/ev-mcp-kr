...
## Stage 10.5 — 인터랙티브 web 대시보드 (2026-05-11 ~ 2026-05-12)

**범위:** ADR-001 의 결정 범위 밖. Stage 10.2 가 만든 Python MCP 툴(자연어 분석)의 자매 도구로, 같은 Parquet 위에서 **시각적 분석 / 인사이트 도구** 를 만듦. DuckDB-WASM 으로 브라우저가 직접 query — 서버 부담 0, 빌드 step 0, npm 의존성 0.

### 산출물 (8 페이지 + 공유 인프라)

| 페이지 | 역할 |
|---|---|
| `web/index.html` | 메인 대시보드 — 전체 508k rows (KPI 8 + 차트 7 + 인사이트 5) |
| `web/highway/index.html` | 고속도로 휴게소 차트 (`kind_detail='C001'` 1979대) |
| `web/highway/map.html` | Leaflet 지도 (622곳 마커, 노선 필터) |
| `web/highway/operators.html` | 운영자 deep dive (12 ranking + 히트맵 + 진입 연도 + 산점도) |
| `web/highway/activity.html` | 활성도 분석 (시간대 + 회전율 + 노선별 + 휴면 분포) |
| `web/highway/route.html` | 노선 deep dive (드롭다운 + 휴게소 list + 거리 + 사각지대) |
| `web/highway/station.html` | 휴게소 상세 drill-down (검색 + 충전기 list + mini 지도) |
| `web/highway/compare.html` | 비교 분석 (entity 2~4 + radar + 자동 인사이트) |

| 공통 | 라인 |
|---|---|
| `web/styles.css` | ~820줄 (다크 모드 + KPI + 차트 카드 + chip + 표 + 지도 + popup) |
| `web/README.md` | 운영/배포/한계 가이드 |

전체 약 5,200줄. JS bundle 7 파일.

### 핵심 결정

- **순수 정적 자산.** package.json·빌드 step 없음. ES module + CDN 만. Cloudflare Pages / GitHub Pages 등 어디든 배포 가능.
- **DuckDB-WASM in browser.** 백엔드 X. 클라이언트가 14.4MB Parquet 직접 fetch + SQL 실행. ADR-001 Alt-1 (Workers 내부 DuckDB) 의 한계가 *브라우저* 환경에선 사라짐 — 메모리·CPU 자유.
- **innerHTML 사용 0.** 모든 동적 DOM 은 `createElement` + `textContent` + `append`. XSS 잠재성 차단 (보안 후크 기준 충족).
- **요일별 분석 의도적 생략.** 단일 스냅샷의 last_tsdt 가 목요일에 편향 (1634/1977) → misleading. 데이터 정직성 우선.
- **stat 코드 정의 web 측도 일관.** Stage 10.4 의 `DOWNTIME=('1','4','5')` + `UNMONITORED='9'` 분리를 모든 페이지 SQL에 적용.
- **drill-down 경로 2개.** 노선 페이지 휴게소 차트 클릭 → station. 지도 popup "상세 보기 →" → station. station 의 mini 지도 → 다른 station (chain).
- **URL state.** `?hw=`, `?id=`, `?type=&entities=` 로 deep link 공유. localStorage X.
- **chip-based entity picker** (비교 페이지). 2~4 동시 선택. 추천 preset (top 4 / 신규 / 기존).
- **자동 인사이트 generator** (비교 페이지). 각 KPI 별 격차 큰 순 정렬 + 모순 패턴 ("규모 1위 ≠ 출력 1위") 자동 추출.

### 발견된 인사이트 (실 데이터 자동 추출)

자연어 스모크 (Stage 10.4) + 이제 시각적 스모크:

- **고속도로 vs 일반 격차.** 고속도로 1979대(0.4%) 가 완전히 다른 인프라 — DC 99.2% vs 일반 9.9%, 평균 196.8 kW vs 16.8 (12×).
- **시장 구조: 1휴게소-1운영자 독점.** 622곳 모두 단일 운영자. 시내 충전소(멀티 운영자) 와 결정적 차이.
- **운영자 전략 분화.** ME (환경부) = 분산형 (6개 노선), SG (SK시그넷) = 서해안 집중 (66대), BE (브라이트) = 중부내륙 (34대).
- **세대 교체.** 2017~2022 기존 강자 (ME/KP) → 2023+ 폭발 진입 (ST 264대, SG 92→120, EC 5→116).
- **고속도로 활성도 매우 높음.** 7일 내 사용 94.6%, 30일 내 98.4%. 60일+ 휴면 25대(1.3%) 만.
- **회전율 차이.** AC완속 30.8분 vs 350+kW 초급속 20.3분 — 1.5× 빠름.
- **점심 피크 명확.** 12~13시 전체의 70%. 새벽 0~5시 1.8%.

### 검증

- 사용자 자연어 스모크 (Stage 10.4 의 1차 발견 + 시각적 확인)
- 브라우저 호환 (Chrome/Firefox/Safari 모던)
- DuckDB-WASM 1.28 + Chart.js 4.4 + Leaflet 1.9 — 모두 LTS 또는 stable
- 7 페이지 × 약 4~6 차트 = 30+ 시각화. 모두 동일 패턴 (DuckDB query → BigInt→Number 변환 → Chart.js/Leaflet 렌더)

### 한계 / 다음 단계

- **첫 로드 ~30MB** (DuckDB-WASM 15MB + Parquet 14.4MB + Chart.js 200KB). 이후 캐시.
- **시계열 분석 부재** — 단일 스냅샷. Stage 10.1 (R2 일별 export) 진입 후 가능.
- **요일 분석 미구현** — 위 동일 이유 (편향).
- **자연어 query 부재** — 의도적. MCP 서버가 그 역할. web/ 은 시각적 도구.
- **공유 URL 일부만** — 노선/휴게소/비교 페이지만 deep link. 나머지는 페이지 단위만.
...
- 2026-05-11 ~ 2026-05-12 — Stage 10.5 web 대시보드 8 페이지 구축. ADR 결정 범위 밖, 같은 Parquet 위의 시각적 자매 도구. DuckDB-WASM + Chart.js + Leaflet. 정체성 = "인사이트 제공 도구".
